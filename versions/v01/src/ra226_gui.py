...
DEFAULT_FILE = os.path.join(_base_dir(), "226Ra_En_Area.txt")   # auto-load if present
...
_TIP_RAD_MC = (
    ...
    "Mean of f_radware(E₀) over MC refits,\n"
    "each with N and I resampled from N(μ, σ).\n"
    "Value is in arbitrary units (depends on geometry)."
)
_TIP_RAD_DEFF = (
    ...
    "7-parameter Radware function."
)
_TIP_RAD_BF = (
    "Radware best-fit efficiency: f_radware(E₀, *popt)\n"
    "where popt is the 7-parameter least-squares fit.\n\n"
    "Use the MC σ row above as the uncertainty."
)
...
class CalibrationEngine:
    ...
    def calibrate_efficiency(self, progress_cb=None):
        ...
        bounds_rw  = ([-np.inf]*6 + [1e-4], [np.inf]*7)
        ...
        # ── Radware best-fit — multi-start ────────────────────────────
        # All candidates now have a1, a4 = log(ε) values (negative, since
        # ε < 1 for relative efficiencies in our scale).  This matches the
        # Radford C-code parameterization.
        if progress_cb: progress_cb(8, "Best-fit Radware curve …")
        p0_parset = _radware_p0(self.E, eff)               # Radford parset
        p0_polyfit = _radware_p0_polyfit(self.E, eff)      # polyfit fallback
        rw_candidates = [
            p0_parset,
            p0_polyfit,
            # variations on g (transition sharpness)
            [p0_parset[0], p0_parset[1], 0.0,
             p0_parset[3], p0_parset[4], 0.0,  5.0],
            [p0_parset[0], p0_parset[1], 0.0,
             p0_parset[3], p0_parset[4], 0.0, 25.0],
            # gentler slope seed
            [p0_parset[0], 1.0,  0.0,
             p0_parset[3], -0.5, 0.0, 15.0],
        ]
        popt_rw = _multistart(f_radware, self.E, eff, deff,
                              rw_candidates, bounds_rw)
        # Validate before accepting — a degenerate best-fit (NaN, g≤0, or
        # absurdly large coefficients) would poison every MC iteration
        # downstream.  If invalid, skip Radware MC entirely.
        if (popt_rw is not None
                and np.all(np.isfinite(popt_rw))
                and popt_rw[6] > 0
                and np.all(np.abs(popt_rw[:6]) < 200)):
            self.radware_popt = popt_rw
            res_r = eff - f_radware(self.E, *popt_rw)
            self.radware_rms   = float(np.sqrt(np.mean(res_r**2)))
            self.radware_ndf   = len(self.E) - 7
        ...
        rng = np.random.default_rng(SEED)
        store_kfr = []; store_rw = []
        last_rw   = popt_rw    # rolling warm-start for Radware
        n_bad     = 0          # count rejected pathological samples
        MC_MAXFEV = 1500
        MC_TOL    = 1e-5       # ftol = xtol = gtol for MC fits

        def _last_rw_valid(p):
            return p is not None and np.all(np.isfinite(p)) and p[6] > 0
        ...
        for k in range(N_MC_EFF):
            ...
            # KFR — LM, no bounds (warm-start keeps c≤0, d≥0 by inertia).
            # If LM wanders out of the physical region the result will be
            # silently rejected via the finiteness check.
            try:
                pp, _ = curve_fit(
            ...
            # Radware — three-tier fallback:
            #   1. LM with warm-start  (~1.5 ms, ~82% success)
            #   2. LM with best-fit    (~1.5 ms, catches most LM failures)
            #   3. TRF with best-fit   (~10 ms, kink-tolerant last resort)
            # On a typical run this triggers tier-1 only.
            if self.radware_popt is not None:
                seed_a = last_rw if _last_rw_valid(last_rw) else popt_rw
                pp_r = None
                # Tier 1 + 2: LM
                for _p0_rw in (seed_a, popt_rw):
                    if _p0_rw is None:
                        continue
                    try:
                        cand, _ = curve_fit(
                            f_radware, self.E, eff_s, p0=_p0_rw,
                            sigma=deff, absolute_sigma=True,
                            maxfev=MC_MAXFEV, method="lm",
                            ftol=MC_TOL, xtol=MC_TOL, gtol=MC_TOL,
                        )
                ...
                if pp_r is None:
                    try:
                        cand, _ = curve_fit(
                            f_radware, self.E, eff_s, p0=popt_rw,
                            sigma=deff, absolute_sigma=True,
                            bounds=bounds_rw, maxfev=MC_MAXFEV, method="trf",
                            ftol=MC_TOL, xtol=MC_TOL, gtol=MC_TOL,
                        )
                        if (np.all(np.isfinite(cand)) and cand[6] > 0
                                and np.all(np.abs(cand[:6]) < 200)):
                            pp_r = cand
                    except Exception:
                        pass
    ...
    def predict_efficiency(self, E_val):
        ...
        if self.params_radware is not None and len(self.params_radware) > 0:
            ...
            v_r = f_radware(E, p[:,0], p[:,1], p[:,2], p[:,3],
                               p[:,4], p[:,5], p[:,6])
            ...
            if self.radware_popt is not None:
                rw_bf_raw = float(f_radware(E, *self.radware_popt))
...
class App(tk.Tk):
    ...
    def __init__(self):
        super().__init__()
        self.title("Energy & Efficiency Calibration")
        self._theme_name = "dark"
        for k, v in _DARK.items(): setattr(self, k, v)
        self.configure(bg=self.DARK)
        self.geometry("1540x900")
        self.resizable(True, True)
        # window icon (ICO embedded next to the executable)
        _ico = os.path.join(_base_dir(), "Ra226_Calibration.ico")
    ...
    def _draw_efficiency(self):
        e = self.engine; E = e.E; eff = e.eff; deff = e.deff
        ax = self.ax_eff; ax.cla(); self._style_ax(ax)
        _plt.setp(ax.get_xticklabels(), visible=False)
        ax.set_ylabel("ε = N/I  (a.u.)", color=self.TEXT, fontsize=9)
        ...
        # Pin y-axis to data range regardless of band extent
        ...
        ax_r.set_ylabel("Δε", color=self.TEXT, fontsize=8)
        ax_r.set_xlabel("E  (keV)", color=self.TEXT, fontsize=9)
        res_k = eff - f_kfr(E, *e.eff_popt)
        ...
        ax_r.axhline( e.eff_rms,  color=self.EFF_C, lw=0.8, ls=":", alpha=0.7)
        ax_r.axhline(-e.eff_rms,  color=self.EFF_C, lw=0.8, ls=":", alpha=0.7)
        ax_r.text(0.01, 0.82, f"KFR RMS={e.eff_rms:.4g}",
                  transform=ax_r.transAxes, ha="left", fontsize=7, color=self.EFF_C)
        # Collect all residuals for a symmetric, data-driven y-limit
        all_res = [res_k]
        if e.radware_popt is not None:
            res_r = eff - f_radware(E, *e.radware_popt)
            ax_r.errorbar(E, res_r, yerr=deff, fmt='s', ms=4,
                          color=self.RAD_C, ecolor=self.MUTED, capsize=2,
                          elinewidth=1, label="Radware")
            ax_r.text(0.01, 0.60, f"Rad RMS={e.radware_rms:.4g}",
                      transform=ax_r.transAxes, ha="left", fontsize=7,
                      color=self.RAD_C)
            all_res.append(res_r)
        # Symmetric residual limits with headroom
        res_arr = np.concatenate(all_res)
        rmax = float(np.nanmax(np.abs(res_arr) + deff)) * 1.45
        if rmax < 1e-30:
            rmax = 1.0
    ...
    def _reset_eff_mc_placeholder(self):
        ...
        ax.set_xlabel("ε  (a.u.)", color=self.TEXT, fontsize=8)
    ...
    def _write_cal_to_file(self):
        ...
        if e.eff_ready:
            ...
            out += ["KFR:  ε(E) = (a·E + b/E) · exp(c·E + d/E)",
                    ...
                    ""]
            if e.radware_popt is not None:
                a1,a2,a3,a4,a5,a6,g = e.radware_popt
                out += ["Radware:  ln ε = [(a1+a2·x+a3·x²)^-g + (a4+a5·y+a6·y²)^-g]^(-1/g)",
        ...
        if e.eff_ready:
            ...
            eff_fit_r = (f_radware(e.E, *e.radware_popt) if has_rw
                         else [float("nan")] * e.n)
            for i in range(e.n):
                out.append(
    ...
    def _query_eff(self):
        ...
        def _ns(x, fmt=".5g"):
            return (f"{x:{fmt}}" if isinstance(x, float) and np.isfinite(x)
                    else "—")

        self._eff_val_mc.set(_ns(kfr_mean))
        self._eff_derr.set(f"±{kfr_std:.5g}" if np.isfinite(kfr_std) else "—")
        self._eff_val_bf.set(_ns(kfr_bf))
        self._eff_rw_val_mc.set(_ns(rw_mean))
        self._eff_rw_derr.set(f"±{rw_std:.5g}" if np.isfinite(rw_std) else "—")
        self._eff_rw_val_bf.set(_ns(rw_bf))

        self.btn_eff_clr.config(state="normal")
        self.btn_clr.config(state="normal")
        try:
            self._draw_eff_query(E_val, kfr_mean, kfr_std, kfr_bf,
                                 rw_mean, rw_std, rw_bf)
        except Exception as exc:
            self._status(f"❌  Plot error: {exc}", self.RED)
            self.btn_eff_query.config(state="normal"); return
        finally:
            # Always re-enable the button regardless of draw success/failure
            self.btn_eff_query.config(state="normal")

        # Append query to result file (use _ns for safe formatting)
        now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        self._append_query_to_file(
            f"[{now}]  EFFICIENCY QUERY\n"
            f"  E₀ = {E_val:.4f} keV\n"
            f"  KFR    ε(MC mean)  = {_ns(kfr_mean, '.6e')} ± {_ns(kfr_std, '.6e')}\n"
            f"  KFR    ε(best-fit) = {_ns(kfr_bf, '.6e')}\n"
            f"  Radware ε(MC mean)  = {_ns(rw_mean, '.6e')} ± {_ns(rw_std, '.6e')}\n"
            f"  Radware ε(best-fit) = {_ns(rw_bf, '.6e')}\n\n")

        kfr_part = (f"  KFR={kfr_mean:.4g}±{kfr_std:.4g}"
                    if np.isfinite(kfr_mean) else "  KFR=N/A")
        rw_part  = (f"  Rad={rw_mean:.4g}±{rw_std:.4g}"
    ...
    def _draw_eff_query(self, E_val, kfr_mean, kfr_std, kfr_bf=None,
                        rw_mean=None, rw_std=None, rw_bf=None):
        ...
        # KFR: only plot when the value is finite — passing inf/nan to errorbar
        # corrupts the axes' container list and breaks subsequent queries.
        ...
        # MC histogram — overlay KFR and Radware distributions
        e   = self.engine
        ax_mc = self.ax_eff_mc; ax_mc.cla(); self._style_ax(ax_mc)
        ax_mc.set_xlabel("ε  (a.u.)", color=self.TEXT, fontsize=8)
        ...
        def _mc_bins(samples, n=60):
            ...
            if len(s) < 2:
                return 10          # fall back to auto integer
            lo, hi = np.percentile(s, [0.5, 99.5])
            if lo >= hi:           # all samples identical
                lo, hi = s.min(), s.max()
            if lo >= hi:           # truly degenerate
                return 10
            return np.linspace(lo, hi, n + 1)

        any_hist = False
        kfr_mc = f_kfr(float(E_val), e.params_eff[:,0], e.params_eff[:,1],
                       e.params_eff[:,2], e.params_eff[:,3])
        ...
        if rw_ok and e.params_radware is not None and len(e.params_radware) > 0:
            p = e.params_radware
            rw_mc = f_radware(float(E_val), p[:,0], p[:,1], p[:,2],
                              p[:,3], p[:,4], p[:,5], p[:,6])
